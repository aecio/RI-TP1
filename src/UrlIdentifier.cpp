...
int main(int argc, char** argv) {
	//args
	string usage_error_message =
			"Usage: <complete_path_to_consultas_file> <results-directory>";
...
void gerarFilesID() {
	...
	while (!input_consultas.eof()) {
		...
		if (!output.is_open()) {
			cerr << "ERRO  - Nao foi possivel abrir o arquivo:"
					<< (consultasDirectory + query + ".num") << endl;
			exit(1);
		}
		string url;
...
bool ProcessArgs(const int argc, const int number_of_args,
		bool print_erro_message, std::string erro_message) {
